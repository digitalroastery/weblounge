...
import static ch.entwine.weblounge.bridge.oaipmh.OaiPmhUtil.toUtc;

import ch.entwine.weblounge.bridge.oaipmh.Granularity;
import ch.entwine.weblounge.bridge.oaipmh.OaiPmhConstants;
...
/**
 * The repository client provides low level methods to talk to an OAI-PMH
 * repository.
 */
public final class OaiPmhRepositoryClient {
  ...
  /**
   * Create and setup a new harvester for the given repository
   * <code>baseUrl</code>.
   */
   ...
   */
  public ListRecordsResponse listRecords(String metadataPrefix,
      Option<Date> from, Option<Date> until, Option<String> set) {
    String queryParams = join("verb=" + OaiPmhConstants.VERB_LIST_RECORDS, "metadataPrefix=" + metadataPrefix, from.map(mkQueryParamDate("from")).getOrElse(""), until.map(mkQueryParamDate("until")).getOrElse(""), set.map(mkQueryParam("set")).getOrElse(""));
   ...
   */
  public ListRecordsResponse resumeListRecords(String metadataPrefix,
