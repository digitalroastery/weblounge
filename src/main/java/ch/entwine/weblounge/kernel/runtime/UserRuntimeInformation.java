 ...
 */
public class UserRuntimeInformation implements RuntimeInformationProvider {
  ...
  public String getRuntimeInformation(Site site, User user, Language language) {
    if (user == null)
      return null;
    String userXml = user.toXml();

    // Filter out the root tag
    Pattern p = Pattern.compile("^<([^>]*)>(.*)</\\1>$");
    Matcher m = p.matcher(userXml);
    if (m.matches())
      userXml = m.group(2);

    // Remove the password
    userXml = userXml.replaceAll("<password.*</password>", "");
    userXml = ConfigurationUtils.processTemplate(userXml, site);
    return userXml;
  }
