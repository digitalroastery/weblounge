...
public enum ScanOrder {
  ...
  /** Scan the top field first */
  TopFieldFirst,

  /** Scan the bottom field first */
  BottomFieldFirst;

  /**
   * Creates a new scan order object from the given argument.
   * 
   * @param value
   *          the scan order as a string
   * @return the scan order
   * @throws IllegalArgumentException
   *           if no such scan order exists
   */
  public static ScanOrder fromString(String value) {
    return ScanOrder.valueOf(StringUtils.capitalize(value));
  }
