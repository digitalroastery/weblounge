...
public class Importer {
	 ...
	 * @param rootCollection the root collection to import
	 * @param quiet <code>true</code> to suppress output
	 */
	...
	public void importPages() throws Exception {
		File siteRoot = new File(PathUtils.concat(new String[] { srcDir, "/db/weblounge/sites", site, "site" }));
	...
	public void importResources() throws Exception {
		File siteRoot = new File(PathUtils.concat(new String[] { srcDir, "/db/weblounge/sites", site, "resources" }));
	...
	public void importUsers() throws Exception {
		File siteRoot = new File(PathUtils.concat(new String[] { srcDir, "/db/weblounge/sites", site, "user" }));
   ...
   * @throws Exception
	...
	/**
	 * Traverses the directory structure and calls the callback on every
	 * file and folder.
	 * 
	 * @param root the root directory
	 * @param callback the callback
	 * @param clipboard the shared memory
			...
			while (!directories.empty()) {
				...
				for (int i=0; i < files.length; i++) {
					try {
						...
						} else if (!"__contents__.xml".equals(files[i].getName()) && !".DS_Store".equals(files[i].getName())) {
