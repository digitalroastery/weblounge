...
/**
 * The action bundle contains at least one but maybe more concrete actions,
 * supporting multiple output methods.
 */
public class ActionHandlerBundle extends LocalizableContent implements PoolListener, SiteListener {
   ...
   * Adds the action definition to this action bundle. The <code>
	 * action</code>
   * argument is used to instantiate actions on demand, the <code>config</code>
   * is then used to configure the newly created actions.
   * 
   * @param actionClass
   *          the action class
   * @param config
  ...
  /**
   * Returns the supported output methods. The meaning of methods is the
   * possible output format of a action. Therefore, the methods usually include
  ...
  /**
   * Returns an instance of the action, supporting the requested rendering
   * method or <code>null</code> if no such action could be instantiated.
   * <p>
   * Make sure to return the action after the rendering request has been
   * accomplished by using <code>returnAction</code> to support action pooling.
   * 
   * @param method
  ...
  /**
   * Returns the url that is being targeted by default when executing this
   * action. You may always overwrite this setting by passing a request
   * parameter of name <tt>url</tt>.
   * 
  ...
  /**
   * Returns a action that has previously been leased from the local rendering
   * pool.
   * 
   * @param action
   *          the returned action
  ...
  /**
   * Creates a action, suitable for the required rendering method. If no such
   * action could be created, then this method returns <code>null</code>.
   * 
   * @param method
