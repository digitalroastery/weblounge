...
import ch.o2it.weblounge.common.impl.security.GroupRegistryImpl;
import ch.o2it.weblounge.common.impl.security.RoleRegistryImpl;
import ch.o2it.weblounge.common.impl.security.SecurityManagerImpl;
import ch.o2it.weblounge.common.impl.url.PathSupport;
import ch.o2it.weblounge.common.impl.url.UrlRegistryImpl;
...
/**
 * The <code>Site</code> object represents one of multiple sites
 * managed by the weblounge system.
 *
...
public class SiteImpl implements Site {
   ...
   /**
	* Returns the description of this site, e.g. "World Floorball Championships 2004".
	* 
	* @param l the language used to describe the site
	...
	*/
	...
	/**
	 * @see ch.o2it.weblounge.common.api.util.Customizable#getOption(java.lang.String, java.lang.String)
	 */
	 ...
	 * 
	 * @param listener the user listener to add
	 */
	...
	/**
	 * Adds a <code>PageListener</code> to the page registry, who will
	 * be notified about new, moved, deleted or altered pages.
	 * 
	 * @param listener the new page listener
	 */
	 ...
	 * 
	 * @param listener the page listener
	 */
	...
	/**
	 * Removes the site channel <code>channel</code> from the subscription
	 * list.
	 * 
	 ...
	 * 
	 * @param path the virtual (site-relative) path
	 ...
	 * 
	 * @param path the virtual path relative to the site
	 * @param webapp <code>true</code> to preprend the webapp url
	...
	public String getVirtualPath(String path, boolean webapp) {
		...
		if (webapp) url = UrlSupport.concat(Env.getURI(), url);
		return url;
	}

	/**
	 * Returns the servername of this page. This method will return the complete
	 * hostname as found in the <code>&lt;name&gt;</code> section
	 * of <code>site.xml</code>.
	 * 
	...
	/**
	 * Returns the absolute link which can be used to reach this page. This method will
	 * return the complete hostname as found in the <code>&lt;name&gt;</code> section
	 * of <code>site.xml</code> concatenated with the weblounge mountpoint.
	 * 
	 ...
	 * <p>
	 * The method returns the collection either if it exists or can be created (<code>
	 * create</code> is <code>true</code>.
	 * 
	 * @param path the site relative path
	 * @param create <code>true</code> to create the collection
	 * @return the collection or <code>null</code> if the collection does not exist
	 */
	 ...
	 * 
	 * @param c the collection to be returned
	 */
	...
	/**
	 * Returns <code>true</code> if pages should be stored in an internal
	 * page cache. Using the cache results in better overall performance since
	 * multiple database lookups for a page may be avoided.
	 * 
	...
	/**
	 * Returns <code>true</code> <code>language</code> is
   ...
   /**
    * Returns the server names that will lead to this site. A servername is
    * the first part of a ur. For example, in <tt>http://www.o2it.ch/weblounge</tt>,
   ...
   /**
    * Returns the mountpoints of this site. Mountpoints define a partition
    * and a url, where this partition is mounted.
    * 
	...
	*/
	...
	/**
	 * Returns the site modules that are currently associated with this
	 * site. Note that the modules may be active or inactive.
	 * 
	...
	/**
	 * Returns the site module with the given identifier or <code>null</code>
	...
	*/
	...
	/**
	 * Returns this site's renderer registry which keeps track of the defined renderer
	 * bundles.
	 * 
	...
	/**
	 * Returns this site's request handler registry which keeps track of the defined
	 * request handler vor servlets etc.
	 * 
	...
	/**
     * Returns the <code>UserRegistry</code> of this site
	 * where all users are stored. The user registry may
	 * also be used to verify that a user has certain roles.
	 * 
	...
	/**
     * Returns the <code>GroupRegistry</code> of this site
	 * where all groups are stored.
	 * 
	...
	/**
     * Returns the <code>RoleRegistry</code> of this site
	 * where all roles are stored.
	 * 
	...
	/**
	 * Returns the site specific error handler which is used to display error messages
	 * in pages and pagelets.
	 * 
    ...
    */
	...
	/**
	 * Returns the image style with the given identifier or <code>null</code>
	 * if no such style is found.
	 * 
   ...
   /**
    * Returns a class loader suitable for loading module
	...
	*/
	...
	/**
	 * Returns the site service identified by <code>service</code>
	 * or <code>null</code> if no such service is available.
	 * 
	...
	/**
	 * Returns the requested service if it can be found in the registry (and
	 * it is enabled, in case that <code>enabledOnly</code> is <code>true</code>).
	 * 
	 * @param id the service identifier
	 * @param enabledOnly <code>true</code> to return enabled services
	 * 	only
   ...
   /**
    * Returns a hashcode for this site object, which is the
	* unique key taken from the community database.
	*/
	public int hashCode() {
		return id_.hashCode();
	}
	...
	*/
	...
	/* -------------------------------------------------------------
	 * I N T E R F A C E   ModuleListener
	 * -------------------------------------------------------------
	 */

	/**
	 * Callback to notify this site that an associated module has been
	 * started.
	 * 
	...
	/* -------------------------------------------------------------
	 * I N T E R F A C E   RequestListener
	 * -------------------------------------------------------------
	 */
	 ...
	 */
	public void requestStarted(WebloungeRequest request, WebloungeResponse response) {
	 ...
	 * @param response the servlet response
	 */
	public void requestDelivered(WebloungeRequest request, WebloungeResponse response) {
	 ...
	 */
	public void requestFailed(WebloungeRequest request, WebloungeResponse response, int reason) {
		fireRequestFailed(request, response, reason);
	}

	/* -------------------------------------------------------------
	 * I N T E R F A C E   UserListener
	 * -------------------------------------------------------------
	 */
	
    /**
     * This  method is called if the user moves from one url to another. Note
     * that moving does not include calling actions. Only movements that are
     * detected by the <code>SimpleRequestHandler</code> are noted.
     * 
     * @param user the moving user
     * @param url the url that the user moved to
     */
	...
	/* -------------------------------------------------------------
	 * I N T E R F A C E   PageListener
	 * -------------------------------------------------------------
	 */
	...
	/**
	 * Notifies the listener about a change in the list of keywords at url <code>url</code>.
	 * 
	 * @param url the page url
	...
	/**
	 * The dispatching method, which takes a <code>HttpServletRequest</code>
	 * and dispatches it to the site (which will in turn pass it on to a jsp or
	 ...
	 * the site. This method then starts a new <code>XMLConfigurator</code> on
	 * the configuration file to stay updated about configuration changes.
	 * 
	...
	/**
	 * Initializes and starts the site. This is done after configuration and mainly registers
	 * and initializes the various site registries.
	 */
	public void start() {
	...
	/**
	 * Method to shut down this site. This includes unloading all modules, stopping
	 * the site services and sending a <code>shudown</code> event to registered
	 * site listeners.
	 */
	public void stop() {
	...
	/**
	 * Method to restart this site. This includes restarting all modules and services
	 * and sending a <code>restart</code> event to registered site listeners.
	 */
	public void restart() {
...
//					log_.error("Unable to start service '" + service + "' of site '" + this + "':" + e.getMessage());
//				} catch (ServiceDependencyException e) {
//					log_.error("Unable to start service '" + service + "' of site '" + this + "' due to cirular dependencies.");
//				}
//			}
//			
//			// Restart Modules
...
//		}
	...
	/**
	 * Method to fire a <code>startRequest()</code> message to all
	 * registered <code>RequestListeners</code>.
	 * 
	 * @param request the starting request
	 * @param response the request response
	 */
	protected void fireStartRequest(WebloungeRequest request, WebloungeResponse response) {
	...
	/**
	 * Method to fire a <code>requestDelivered()</code> message to all
	 * registered <code>RequestListeners</code>.
	 * 
	 * @param request the delivered request
	 * @param response the request response
	 */
	protected void fireRequestDelivered(WebloungeRequest request, WebloungeResponse response) {
		for (int i=0; i < requestListeners.size(); i++) {
			RequestListener listener = (RequestListener)requestListeners.get(i);
	...
	/**
	 * Method to fire a <code>requestFailed()</code> message to all
	 * registered <code>RequestListeners</code>.
	 * 
	 * @param request the failing request
	 * @param error the error code
	 */
	protected void fireRequestFailed(WebloungeRequest request, WebloungeResponse response, int error) {
		for (int i=0; i < requestListeners.size(); i++) {
			RequestListener listener = (RequestListener)requestListeners.get(i);
	 ...
	 * @param url the url where the user moved to
	 */
	 ...
	 * @param user the editing user
	 */
	protected void firePageRendererChanged(WebUrl url, Renderer newRenderer, Renderer oldRenderer, User user) {
		for (int i=0; i < pageListeners_.size(); i++) {
	 ...
	 * @param user the editing user
	 */
	protected void firePageTypeChanged(WebUrl url, String newType, String oldType, User user) {
		for (int i=0; i < pageListeners_.size(); i++) {
	 ...
	 * @param user the editing user
	 */
	protected void firePageLayoutChanged(WebUrl url, Layout newLayout, Layout oldLayout, User user) {
		for (int i=0; i < pageListeners_.size(); i++) {
	 ...
	 * @param user the editing user
	 */
	protected void firePageKeywordsChanged(WebUrl url, String[] newKeywords, String[] oldKeywords, User user) {
		for (int i=0; i < pageListeners_.size(); i++) {
