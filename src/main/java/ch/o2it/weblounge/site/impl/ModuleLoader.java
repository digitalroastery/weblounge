...
/**
 * This watchdog keeps watching the <code>modules</code> folder to be able to
...
class ModuleLoader extends DirectoryWatchdog {
  ...
  /**
   * Creates a new ModuleLoader which will immediately start searching for
   * modules that have either been added or removed.
   * 
   * @param directory
  ...
  /**
   * Initializes the module watcher by making every module directory, that
   * exists at the beginning, to appear as if it had been added.
   */
  void init() {
  ...
  /**
   * Method is called if a new file has been detected in the modules folder. It
   * then checks if it is a folder and has a module.xml file in it. If so, the
   * new module is being configured and eventually started.
   * 
   * @param moduleDir
