...
class CacheableHttpServletResponse extends HttpServletResponseWrapper {
	...
	/** 
	 * holds the special tee writer that copies the output to the network and
	 * to the cache.
	 */
	 ...
	 * 
	 * @param tx the cached transaction represented by this cacheable response
	 */
	 ...
	 * 
	 * @return a PrintWriter object that can return character data to the
	 * client
	 * @throws IOException if the writer could not be allocated
	 ...
	 * 
	 * @param hnd the handle that produced the cache hit
	 * @param buf the content from the cache
	 */
	 ...
	 * 
	 * @param hnd the handle that produced a cache miss
	 */
	...
	/**
	 * Invaidate the output. Tells the cache writer to stop adding output to
	 * the cache. 
	 */
	...
	/**
	 * Format a date for an HTTP header.
	 * 
	...
	private String formatDate(long date) {
		if (format == null) {
			format = new SimpleDateFormat("EEE, dd MMM yyyy HH:mm:ss 'GMT'",
					Locale.US);
