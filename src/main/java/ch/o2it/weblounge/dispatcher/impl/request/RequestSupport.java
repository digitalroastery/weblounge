...
/**
 * Facade to the <code>HttpRequest</code> object with easy-to-use
 * access to user and language information.
 *
...
public final class RequestSupport {
   ...
   /**
    * RequestSupport is a static class and therefore has no
    * constructor.
    */
     ...
     * <code>true</code>.
     * <p>
     * This method throws a <code>SiteNotFoundException</code>
     * if the site is not found.
     * 
     * @param request the <code>HttpRequest</code>
     * @param forward <code>true</code> if unmatched requests should be forwarded
     * 	to the default site
     * @return the site associated with the current request
     * @throws SiteNotFoundException if the site is not found
     */
    public static Site getSite(HttpServletRequest request, boolean forward) throws SiteNotFoundException {
...
//		
//		if (site == null) {
//    		if (forward && sites.getDefault() != null && !SiteWatchdog.sitesLoading()) {
//				site = sites.getDefault();
//				attribs.setSite(site);
//				if (!redirects.contains(request.getServerName())) {
//					log_.warn("Forwarding requests for host '" + request.getServerName() + "' to default site '" + site + "'");
...
//    	return site;
	...
	/**
	 * Returns the site associated with the current request or the default
	 * site, if the hostname doesn't match any of the loaded sites.
	 * <p>
	 * This method throws a <code>SiteNotFoundException</code>
	 * if the site is not found and no default site has been configured.
	 * 
	 * @param request the <code>HttpRequest</code>
	 * @return the site associated with the current request
	 * @throws SiteNotFoundException if the site is not found
	 */
	public static Site getSite(HttpServletRequest request) throws SiteNotFoundException {
		return getSite(request, true);
	}

	/**
	 * Sets the url. This method is used to tweak the url when passing on requests
	 * to a different handler, e. g. if an action has a target url defined which should
	 * be handled by the page handler, but not with the original action url.
	 * 
	...
	/**
	 * Sets the url and version. This method is used to tweak the url when passing on requests
	 * to a different handler, e. g. if an action has a target url defined which should
	 * be handled by the page handler, but not with the original action url.
	 * 
   ...
   /**
    * Returns the request url, which corresponds to the so called domain.
	* The url provides information on the called site, the server name, the
	* webapp path and the webapp-relative url itself.
	* 
	...
	*/
	...
	/**
	 * Sets the version. This method is used to tweak the version when passing on requests
	...
	/**
	 * Returns the request version. This method returns the version if set
   ...
   /**
	...
	/**
	 * Returns the version for the given version identifier. Available versions are:
	 * <ul>
	 * 	<li>{@link #LIVE}</li>
	 * 	<li>{@link #WORK}</li>
	 * 	<li>{@link #ORIGINAL}</li>
