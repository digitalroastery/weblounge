...
public class Http11Utils implements Http11Constants {
	...
	/**
	 * Constructs an HTTP "Allow" response header for the allowed request
	 * methods.
	 *  
	 ...
	 * <li>POST
	 * </ul>
	 * The OPTIONS request method is handled transparently.
	 * <br>
	 * If the check succeeds, the caller can assume that the request methods is
	 * one of the default methods and should be handled accordingly.
	 * <br>
	 * If the check fails, the request is either an OPTIONS request or is not
	 * allowed. But is has already been handled and the caller doesn't need to
	 * do any further request handling.  
	 * 
	 * @param method the request method to check against and handle
	...
	/**
	 * Prepares a response for handling a HEAD request using a GET handler.
	 * <br>
	 ...
	 * discarding all its output!
	 * <br>
	 * NOTE: This method should not be used with the Http11ProtocolHandler,
	 * since it already handles HEAD requests correctly and much more
	 * efficiently!
	 * 
	...
	public static void endHeadResponse(WebloungeResponse resp) {
		DiscardBodyResponse b = 
			(DiscardBodyResponse) ((WebloungeResponseImpl) resp).getResponse();
		b.finishOutput();
	}

	/**
	 * Calulates an ETag for a given modification time.
	 * 
	 * @param modified the modification time in milliseconds since the epoch
	...
	/**
	 * A servlet response that make sure no HTTP response body is sent back
	 * to the client.
	 * 
	 * @version $Revision: 1090 $ $Date: 2009-09-14 19:43:19 +0200 (Mon, 14 Sep 2009) $
	 * @author Daniel Steiner
		...
		public PrintWriter getWriter() throws IOException {
			if (pw == null)
				pw = new PrintWriter(new OutputStreamWriter(
					os, getResponse().getCharacterEncoding()));
			return pw;
		}
	}
	...
	private static class NullOutputStream extends ServletOutputStream {
		...
		/** the number of bytes that habe been written to this stream */
