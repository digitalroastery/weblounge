...
/**
 * This handler is registered with the site dispatcher. If a request is targeted to
 * a registered action, it will answer <code>true</code> to the
...
public class ActionRequestHandler implements RequestHandler, Http11Constants {
    ...
    */
	 ...
	 * 
	 * @param handler the webservice handler
	 */
	public void addHandler(ActionHandlerBundle handler) {
		actions_.put(handler.getIdentifier(), handler);
		log_.debug("Action handler '" + handler + "' registered for " + 
				handler.getConfiguration().getMountpoint());
	}
	
	/**
	 * Removes <code>handler</code> from the list of registered
	 * handlers.
	 * 
	 * @param handler the webservice handler
	 */
	 ...
	 * @param response the webloungeresponse
	...
	public boolean service(WebloungeRequest request, WebloungeResponse response) {
		...
		try {
			...
			boolean cacheActions = cache != null 
				&& !"false".equals(cache.getConfiguration().getOption("actions"))
				&& request.getVersion() == Page.LIVE;
			...
			try {
				if (action.provides(method)) {
					...
					if (rendererId != null) {
						renderer = site.getRenderers().getRenderer(rendererId, request.getOutputMethod());
					} else {
	 ...
	 */
	public void configure(RequestHandlerConfiguration config) throws ConfigurationException {
	...
	/**
	 * Try to determine the target url for the action result. The default
	 * is the current page, but first the target-url request parameter
	 * will be considered, and secondly the config parameter
	 * 
	 * @param request the weblounge request
    ...
    // page. This is why we have to make sure that this target page exists, otherwhise
