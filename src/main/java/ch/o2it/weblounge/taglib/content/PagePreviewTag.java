...
public class PagePreviewTag extends WebloungeTag {
  ...
  /**
   * Sets the path within the partition.
   * 
   * @param value
   *          the path
   */
  ...
  public int doEndTag() throws JspException {
    ...
    try {
      ...
      Page page = PageManager.getPage(partition, path, site, user, SystemPermission.READ, Page.LIVE);
      if (page == null) {
        log_.error("Request for preview of non existing page '" + path + "' in partition '" + partition + "'");
        return EVAL_PAGE;
      }
      WebUrl url = page.getUrl();
      composer_ = ((TemplateConfiguration) page.getRenderer("html").getConfiguration()).getStage();
      ...
      // Read pagelets
      Pagelet[] pagelets = page.getPagelets(composer_);
      ...
      p: for (int i = 0; i < pagelets.length; i++) {
        ...
        try {
          ...
          request.setAttribute(WebloungeRequest.COMPOSER, composer_);
          ...
          if (!(request.getVersion() == Page.WORK) && !pagelet.isPublished()) {
            log_.debug("Skipping pagelet " + i + " in composer " + composer_ + " since it is not yet published");
            continue p;
          }
          ...
          registry = (RendererRegistry) ModuleRegistries.get(RendererRegistry.ID, m);
          if (registry == null) {
            site.getLogger().warn("No renderer registry found for module '" + moduleId + "'");
            continue p;
          }

          // Load renderer
          renderer = registry.getRenderer(rendererId, method);
          if (renderer == null) {
            log_.warn("No suitable renderer '" + moduleId + "/" + rendererId + "' found to handle " + method + " on " + url);
            continue p;
          }
          ...
          String msg = "Exception when processing pagelet '" + pagelets[i].getLocation() + "'";
      ...
      String msg = "Exception when processing composer '" + composer_ + "'";
      log_.error(msg + ":" + t.getMessage());
      log_.warn(msg, t);
      return EVAL_PAGE;
    } finally {
